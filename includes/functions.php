...
if (isset($_COOKIE['idk_session_front'])) {
	...
	$client_key = $_COOKIE['idk_session_front'];

	$query = $db->prepare("
		SELECT client_id, client_show_price, client_show_quantity
		FROM idk_client
	...
	$logged_client_show_price = $client['client_show_price'];
	$logged_client_show_quantity = $client['client_show_quantity'];
	...
	$employee_key = $_COOKIE['idk_session_front_employee'];
	...
	$employee_key = $_COOKIE['idk_session_front_employee_skladistar'];
...
function getEmployeePosition()
{
	...
	$query = $db->prepare("
		...
		WHERE t1.od_group = :od_group AND t1.od_value = t2.employee_position AND t2.employee_id = :employee_id");
...
function getCategories($category_sub = 0)
{
	...
	$cat_query = $db->prepare("
		SELECT category_id, category_name, category_sub
		FROM idk_category
...
//Get last mileage from db for logged client
function getMileageStartTime($employee_id)
{
	global $db;

	//Look for last mileage in db
	$check_mileage_query = $db->prepare("
		SELECT mileage_start_time
		FROM idk_mileage
		WHERE mileage_employee_id = :mileage_employee_id AND mileage_end_time IS NULL");

	$check_mileage_query->execute(array(
		':mileage_employee_id' => $employee_id
	));
...
function getSelectedClientName()
{
	...
	if (isset($_COOKIE['idk_session_front_client'])) {
		...
		if ($number_of_rows != 0) {
			$client = $check_client_query->fetch();
